...
test_num = 1
Network_path = 'Network/gamma09_95_0429'
Network_file_name = '/test.ckpt'
Log_path = 'logs/gamma09_GIF'
SOME_TAG = '_GIF'
...
def Predict_action_value(agent1, agent2, V_pred, W_pred):
    ...
    state_pred = [[V_pred, W_pred, agent1.r, relative_gx, relative_gy, relative_gth, V_max, m11, m12, m13, relative_Px2, relative_Py2, relative_Vx2, relative_Vy2, r2]]
...
def GIF_Path(Path, num, final_time, SAVE_PATH, agent2_goal):
    L = 0.5
    plt.close('all')
    plt.figure(figsize=(12,12))
    ax = plt.gca()
...
def DL_process(DL_database):
    ...
    saver.save(sess, Network_path+Network_file_name)   
...
def RL_process(eposide_num, epsilon, RL_SAVE_PATH):      
    for eposide in range(eposide_num):
        ...
        Path_1[round(time,1)] = Record_Path(agent1, agent2, time)
        Path_2[round(time,1)] = Record_Path(agent2, agent1, time)
        ...
        while(not Check_Goal(agent1, Calculate_distance(resX, resY, 0, 0), resTH)):
            ...
            GIF_Path(Path_1, GIF_num, time, GIF_path, [agent2.gx,agent2.gy,agent2.gth])
            
        lines =  str(agent1_init_state) + ';' + str(agent2_init_state) + '\n'   
        if result == 'Finish':
            Path_1 = Calculate_value(Path_1, Arrived_reward, time)
            f = open(RL_SAVE_PATH + '/Finish.json', 'a+')
            f.writelines(lines)
            f.close()
        elif result == 'TIME_OUT':
            Path_1 = Calculate_value(Path_1, Time_out_penalty, time)
            f = open(RL_SAVE_PATH + '/TimeOut.json', 'a+')
            f.writelines(lines)
            f.close()
        elif result == 'Collision_high':
            Path_1 = Calculate_value(Path_1, Collision_high_penalty, time)
            f = open(RL_SAVE_PATH + '/Collision.json', 'a+')
            f.writelines(lines)
            f.close()
        elif result == 'Collision_low':
            Path_1 = Calculate_value(Path_1, Collision_low_penalty, time)
            f = open(RL_SAVE_PATH + '/Collision.json', 'a+')
            f.writelines(lines)
            f.close()
        elif result == 'Collision_equal':
            Path_1 = Calculate_value(Path_1, Collision_equ_penalty, time)
            f = open(RL_SAVE_PATH + '/Collision.json', 'a+')
            f.writelines(lines)
            f.close()
        else:
